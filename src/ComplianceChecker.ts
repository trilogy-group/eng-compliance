...
import {
  checkHumanName as humanCheckName,
  ruleHumanName as humanRuleName,
  Rule,
  RuleCheck
} from './Rule'
  ...
  async main() {
    ...
    try {
      ...
      reporter.reportCheck(
        'Setup',
        'product is setup for compliance checks',
        {mandatory: true},
        Result.FAIL,
        remediation
      )
      ...
      for (const rule of this.rules) {
        ...
        for (const [checkName, checkFunc] of this.listChecks(rule)) {
          ...
          if (checkOptions == null) {
            throw new Error(
              `Cannot find options for ${checkName} did you declare it with @check?`
            )
          }
