...
export class Branching extends Rule {
  ...
  async checkDefaultBranchName(product: Product) {
    ...
    assert(
      defaultBranchName == 'main' || defaultBranchName == 'master',
      `default branch should be main or master. current is: ${defaultBranchName}`
    )
  }

  @check({mandatory: true})
  async checkNoDevelopBranch(product: Product) {
    const developBranch = product.repo.branches.find(
  ...
  async checkNoNonDefaultProtectedBranch(product: Product) {
    const otherProtected = product.repo.branches.find(
      branch =>
        branch.protected && branch.name != product.repo.defaultBranchName
    )
    assert(
      otherProtected == null,
      `remove or unprotect branch ${otherProtected?.name}`
    )
  }

  @check({mandatory: true})
  async checkdefaultBranchIsProtected(product: Product) {
    const defaultBranch = product.repo.branches.find(
      branch => branch.name == product.repo.defaultBranchName
    )
    assert(
      defaultBranch && defaultBranch.protected,
  ...
  async checkOnlyShortLivedBranches(product: Product) {
    const unprotectedBranches = product.repo.branches.filter(
      branch =>
        !branch.protected && branch.name != product.repo.defaultBranchName
    )
    ...
    for (const branch of unprotectedBranches) {
      ...
      const diff = await this.octokit.repos
        .compareCommits({
      ...
      if (commitDate) {
        ...
        assert(
          branchCreationDate >= earliestBranchAge,
